    ...
    {!data.clients.length ? <Card><CardContent className="workspace-empty"><User size={40} /><h3>{admin ? "Seu primeiro cliente começa aqui" : "Seu acesso está pronto"}</h3><p>{admin ? "Cadastre o nome e o e-mail do cliente. Depois, adicione quantos projetos precisar." : "Ainda não há um cadastro vinculado ao seu e-mail. Peça ao responsável para usar o mesmo e-mail da sua conta."}</p></CardContent></Card> : <>
      ...
      {!selected ? <div className="workspace-empty"><Briefcase size={40} /><h3>{projects.length ? "Nenhum projeto encontrado" : "Nenhum projeto cadastrado"}</h3><p>{projects.length ? "Experimente outro nome ou filtro." : admin ? "Adicione um projeto para este cliente usando Novo projeto." : "Os projetos aparecerão aqui assim que forem cadastrados."}</p></div> : <div className="workspace-layout">
        ...
        <div className="workspace-detail">
          <Card><CardContent className="workspace-project-body"><div className="workspace-project-heading"><div><span className="workspace-stage">{selected.archived_at ? "Arquivado" : deliveryStages[selected.stage]}</span><h2>{selected.title}</h2></div>{admin && <div className="workspace-actions"><Button variant="outline" disabled={busy} onClick={() => setEditor({ kind: "project", value: selected })}><PencilSimple /> Editar projeto</Button><Button variant="outline" disabled={busy} onClick={async () => { if (await save({ action: "archive_project", id: selected.id, version: selected.updated_at, archived: !selected.archived_at })) { setFilter(selected.archived_at ? "all" : "archived"); setEditor(null); } }}>{selected.archived_at ? "Desarquivar projeto" : "Arquivar projeto"}</Button><Button variant="outline" disabled={busy} onClick={() => setDeleteProject(true)}>Excluir projeto</Button></div>}</div>
